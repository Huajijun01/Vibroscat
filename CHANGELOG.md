...
- **大气雾浓度昼夜剖面**：雾浓度改为随太阳高度角变化的两个绝对浓度。原来的 `AIR_FOG_DENSITY` 换成 `AIR_FOG_DENSITY_DAY`（白天浓度，默认 10.0 = 薄雾）与 `AIR_FOG_DENSITY_DUSK`（黄昏浓度，默认 30.0）。太阳高度角先取绝对值折叠到地平线两侧，再用 `smoothstep` 在约 15 度以内从黄昏浓度渐变到白天浓度：日出与日落时雾最浓，白天与夜间回到白天浓度。解析雾合成与极线空气列共用同一个浓度函数（`AirFogMediumScale`），不再各自读一个编译期常量。
...
- **极线空气估计器权重修正**：`EpipolarAirShadowRatio` 逆 CDF 分支每步的权重从 `t_sample` 改成 1。采样点本来就按透过率分布（`u = -log(t_sample)/tau` 就是逆 CDF），再乘一遍等于把采样密度平方，比值收敛到 ∫T²·V/∫T² 而不是 ∫T·V/∫T，加步数也不消失：硬阴影边缘上 200 m 光柱偏 +0.8%、1 km 偏 +3.3%、5 km 偏 +13%、20 km 偏 +19%，4096 步仍停在原处；改完 12 步就落在正确值 0.2% 以内，4096 步逐位吻合。水体那边的同类求积也做了对照：把格宽换成同抖动的单元格边界、或把共享 CDF 挪到均值或最快通道，实测在阴影柱上更差（对比误差最多 +6%），保留原式，注释里记了这次测量。
- **极线空气消光与雾统一出处**：`AirExtinction()` 以前自己另算一份消光，跟 `AirFogMediumAtCamera()` 那份（带雨水雾增量）可以差两个数量级：满湿度下 200 m 光柱雾的 τ 是 2.03，极线这边只有 0.025，半遮挡柱该给 E = 0.734 却返回 0.507。现在 `AirFogMediumAtCamera()` 用 `GetExtinction`（瑞利 + 气溶胶散射 + 气溶胶吸收 + 臭氧）加 `wetness * 0.01` 组成唯一的消光，`AirExtinction()` 直接读这份介质；`mie` 保留 wetness 增量，雨雾照旧散射。清澈空气 200 m 的雾透过率从 0.9748 到 0.9732。`lib/atmosphere/media.glsl` 同时恢复 `GetExtinction` 与 OPAC 物种数据折叠出的气溶胶吸收常量。
- **GI 调试视图修复**：`RSM_DEBUG` 的"滤波辐照度"一档此前与"时域辐照度"输出同一张图（邻域修复排在调试提前返回之后）。现在邻域修复排在调试分支之前，"滤波"一档输出的是光照路径真正使用的修复值。
- **阴影与透叶光统一淡出**：阴影贴图的覆盖淡出此前只写在植物透光（SSS）一项上，由 NDC 边缘、保护深度、视距比三个互相重叠的因子拼成——其中深度因子在保护深度（`SHADOW_DEPTH_SCALE = 1/6`）上取阈值，实际是死代码；直接阴影自身在覆盖边缘没有任何过渡，出界瞬间从滤波结果跳成无遮挡全亮。现在改为单一覆盖置信度：在原始（未扭曲）阴影 clip 空间对 XY/深度两轴取 `smoothstep(SHADOW_SSS_FADE_START, 1.0)` 的乘积，直接阴影按 `mix(1, 滤波值, 覆盖率)` 在约 96→128 m 平滑淡出，植物透光的贴图透射在同一条曲线上交接给 lightmap 天光估计；同时删除了被 `sss_energy × availability` 乘法废掉的交接（出界整项归零，lightmap 终点永不生效）与 44.8–96 m 的额外视距淡出。远端植被不再"先变全亮、再变无透叶光"，出界后透叶光按天光透射继续（与近场向阳叶片一致）。`SHADOW_SSS_FADE_START` 语义扩大为覆盖淡出起点（宏名不变，不重置已存档值），双语标签改为"阴影淡出起点"/"Shadow Fade Start"。无 SSS 档位（LOW/MEDIUM）同样获得阴影平滑淡出；水面半透明的阴影采样边界维持原样，另行处理。顺带修复 `lib/color/dither.glsl` 缺失 `#ifdef COLOR_DITHER` 的 `#endif`（守卫与选项两个条件编译共用一个 `#endif`，Iris 宽容处理下行为不变，严格预处理器会连带吞掉后续内容）。
- **删除植物透光的过渡**：统一淡出落地后的实机观感仍不干净——透射在覆盖淡出带内混合贴图与天光两套估计，天光透射还让边界外的植被永久带辉光。现删除植物透光自身的全部过渡：与阴影滤波同一个 `shadow_coverage > 1e-3` 门内按贴图测量厚度全量渲染，越过覆盖边界整项直接缺席；无混合、无天光交接、无距离渐变，边界外植被就是普通无遮挡着色。阴影自身的平滑淡出保持不变，代价是透叶辉光在覆盖边界有一条硬截止线。
- **删除半影距离柔化**：PCF 半径此前在 10–120 m 视距内按 `SHADOW_DISTANCE_BOOST`（默认 1.5）额外放大，用来掩盖阴影贴图远端的分辨率损失。现删除该逻辑：半影完全由 PCSS 本身的阻挡层深度间隙 × 太阳角半径决定（SSS 加宽与搜索盘上限保留），远端阴影随之变锐，代价是低纹素密度处的远端阴影会更闪。设置项、界面分组、滑条与双语标签一并移除。
...
- **极线步进预算拆分**：水体与空气的极线体积光此前共用一条 `EPIPOLAR_SHADOW_STEPS`（每列光线检查阳光遮挡的步数）。水体列短而对比强，空气列是长距离低对比的雾，同一个步数只能迁就其中一方。现拆成 `WATER_EPIPOLAR_SHADOW_STEPS` 与 `AIR_EPIPOLAR_SHADOW_STEPS` 两项独立设置，取值档位不变（8/12/16/24/32/48/64），都列在“水体 → 体积光栅格”页。水体各档沿用原值（LOW 16，MEDIUM 与 HIGH 24，ULTRA 32），空气各档下调为 LOW 8、MEDIUM 与 HIGH 12、ULTRA 16，声明默认值取 12；空气光柱的步进精度随之下降，换来的是空气积分那一段的步数成本。极线网格仍由两种介质共用，`EPIPOLAR_SLICES` 与 `EPIPOLAR_SAMPLES` 未拆分。旧的 `EPIPOLAR_SHADOW_STEPS` 存档值随之失效，首次进入需要重新选择。
- **水面侧面法线修复**：垂直水面（池壁、瀑布侧面）此前被无条件赋予水平面的波浪法线——正对观看时法线朝上、`ndotv≈0`，侧面变成近乎不透明的镜面，反射朝天，折射偏移基准也错。现在水平面保留原 POM 波浪路径；垂直面改用 `at_tangent` 构建的几何 TBN 把同一波场重参数化到面平面上（无 POM——其步进沿世界竖直位移，对侧面无意义）。`colortex2` 半透明段由存完整法线改为存相对平面的波扰动（xzy 交错、0.25 缩放），blend 的折射偏移随之以面自身法线为基准，平整侧面不再产生虚假位移；水平面与水下的合成结果不变。
