...
- **体积云散射改为 HaringPro**：体积云多重散射模型整体移植 Revelation（Apache-2.0）的 `CloudMultiScatteringApproxHaringPro`，替换原有的三 octave `1/(1+aτ)` 方案与 HanPi phi_fwd 扩散场。方向光相位保留本包原有的三 octave 求和（`Σ c_k·P_k/(1+a_k·τ)`，由 `CLOUD_MS_ATTENUATION`、`CLOUD_MS_CONTRIBUTION`、`CLOUD_MS_ECCENTRICITY` 驱动），透射沿用原有的 `1/(1+τ)`；HaringPro 的闭式各向同性级数 `fms/(1-fms)`（`fms = ω0·(1-exp2(-300σ_t))`）作为独立一项叠加其上；新增地面反弹。上游那个由 density profile 门控的 `msVolume/(1+0.5τ)` 体积项与天光分量都没有移植。日月保持双通道独立光照，云帧打包、光色混合与环境光路径维持原样；上游函数的天光分量未移植。移除 `CLOUD_MS_ATTENUATION`、`CLOUD_MS_CONTRIBUTION`、`CLOUD_MS_ECCENTRICITY`、`CLOUD_MS_DEPTH_POWER`、`CLOUD_MS_DEPTH_BIAS`、`CLOUD_MS_BOUNDARY_CONFIDENCE`、`CLOUD_PHI_INTENSITY`、`CLOUD_PHI_COMPRESSION`，新增 `CLOUD_MS_ALBEDO`(0.999)。相位输入保留本包 dual-lobe HG，未导入 Revelation 的烘焙 Mie 相位表。
