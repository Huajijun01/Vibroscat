...
- **体积云散射改为 HaringPro**：体积云多重散射模型整体移植 Revelation（Apache-2.0）的 `CloudMultiScatteringApproxHaringPro`，替换原有的三 octave `1/(1+aτ)` 方案与 HanPi phi_fwd 扩散场。直射项恢复为严格的 `exp(-τ)` 单次散射，二次以上散射由闭式几何级数 `fms/(1-fms)`（`fms = ω0·(1-exp2(-300σ_t))`）承载于各向同性相位；厚云内部改由唯一的非指数项 `msVolume/(1+0.5τ)` 照亮；新增地面反弹。日月保持双通道独立光照，云帧打包、光色混合与环境光路径维持原样；上游函数的天光分量未移植。移除 `CLOUD_MS_ATTENUATION`、`CLOUD_MS_CONTRIBUTION`、`CLOUD_MS_ECCENTRICITY`、`CLOUD_MS_DEPTH_POWER`、`CLOUD_MS_DEPTH_BIAS`、`CLOUD_MS_BOUNDARY_CONFIDENCE`、`CLOUD_PHI_INTENSITY`、`CLOUD_PHI_COMPRESSION`，新增 `CLOUD_MS_ALBEDO`(0.999)、`CLOUD_MS_VOLUME`(1.0)、`CLOUD_MS_VOLUME_FALLOFF`(0.5)。相位输入保留本包 dual-lobe HG，未导入 Revelation 的烘焙 Mie 相位表。
