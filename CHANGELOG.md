...
这是 Vibroscat 面向 Minecraft 26.2 / Iris 的第二个 Alpha 版本。本版本重点完善了不透明材质反射、体积云多重散射和反射管线的稳定性。
...
- 体积云主视线步进恢复为均匀线性步进；单次散射衰减使用指数透射率。
- `phi_fwd` 增加可调软压缩，默认压缩参数为 `0.5`。
- 修复退化边界交点，补充反射与云散射回归测试。
...
Alpha v0.2.0 improves Lab PBR opaque materials, recursive SSR, material-normal reflection stability, roughness-based SSR/SH blending, and screen-boundary handling. It also ports HPVolumeCloud-style `phi_fwd` multiple scattering with receiver-side integration, bottom/boundary confidence, linear cloud marching, exponential single-scattering attenuation, and configurable soft compression. The legacy reflection filter, confidence/path-length/mip plumbing, and unstable positive-exponent propagation chain were removed.
